...
import {
  getCompletedLessons,
  getUserID,
  getcompleteLessonByUserId,
  assigned,
  getBookByName,
  getLessonByUserIdAndBookIDCompletd,
  getBookName,
} from "../api/auth";
...
const Completed = () => {
  ...
  const handleComplete = async (name) => {
    ...
    const lesson = await getLessonByUserIdAndBookIDCompletd(
      Cookies.get("id"),
      res[0]._id
    );
 ...
 &id=${encodeURIComponent(lesson.data._id)}&name=${encodeURIComponent(
      name
    )}&operation=complete&bk=${encodeURIComponent(
      lesson.data.book_id
