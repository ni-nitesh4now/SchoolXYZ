...
import { getPublications,createPublication,deletePublication,getPublicationByName,getPublicationData,updatePublications,restrictPublication} from '../api/auth'
    ...
    useEffect(() => {
      ...
      const checkNameExist=async(name)=>{
        return new Promise((resolve, reject) => {
            ...
            const Exists = publications.some((b) => b.name.toLowerCase() == name.toLowerCase());
      ...
      const handleDeletePublication = async (pubId) => {
        try {
            ...
            setPublications((prevPublications) => prevPublications.filter((publication) => publication._id !== pubId));
          } catch (error) {
      ...
      const handleUpdate=async()=>{
        try{
            ...
            console.log(response);
            setPublications((prevUsers) => prevUsers.map((user) => (user._id === selectedId ? response.data : user)));
            setShowUpdate(false);
        }
        catch(err){
            console.log(err)
            ...
            <Adminsidenav>
                <Row style={styles.userBody}>
                    <Row style={styles.link}>
                        ...
                        {show && (<div style={adding}>
                            <div style={addOuter}>
                                ...
                                <div style={modalBorder}>
                                    ...
                                    <div style={modalBorder}>
                                        <Form.Control type="text" placeholder="Name" style={inputField} value={publicationName} onChange={(e) => setPublicationName  (e.target.value)}/>
                                    </div>
                                    <div style={modalBorder}>
                                        <Button variant="primary" onClick={handleCreatePublication} style={submitBtn}>
                                             Create
                                         </Button>
                                    </div>
                                </div>
                        ...
                        {showUpdate && (<div style={adding}>
                            <div style={addOuter}>
                                ...
                                <div style={modalBorder}>
                                    ...
                                    <div style={modalBorder}>
                                        <Form.Control type="text" placeholder="Name" style={inputField} value={updateName} onChange={(e) => setUpdateName(e.target.value)}/>
                                    </div>
                                    <div style={modalBorder}>
                       ...
                       {publications && publications.length>0 && (<div style={styles.table}>
                              {publications.map((publication,index)=>(
                                ...
                                <div className='col' style={styles.col2}>
                                    <h6  className='admin' style={{fontSize:'15px',marginBottom:'-2px',marginTop:'6px',fontStyle:'bold'}}>{publication.name}</h6>
                                    <p style={{fontSize:'10px',textAlign:'left'}}>Publication Name</p>
                                </div>
                                <div className='col3' style={styles.col3}>
                                    <a  style={{ ...styles.funcLink ,color: publication.blocked ? 'red' : 'grey', }} onClick={()=>handleRestrict(publication._id)}>
                                    <BiBlock />&nbsp;&nbsp;&nbsp;
                                    </a>
                                    <a  style={styles.funcLink} onClick={()=>handleDeletePublication(publication._id)}>
                                        <AiFillDelete />
                                    </a>
                                    <a style={{ ...styles.funcLink, marginLeft: '15px' }}  onClick={()=>fetchUserForUpdate(publication._id,publication.blocked)}>
                                            <RxUpdate />
                        </a>
                                </div>
                                </div>
