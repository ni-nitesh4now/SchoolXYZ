...
import { deleteBook,getBooks,createBook,getBookData,updateBook ,restrictBook} from '../api/auth';
...
const Createbooks = () => {
    ...
    const handleUpdate=async()=>{
        try{
           ...
           setBooks((prevUsers) => prevUsers.map((user) => (user._id === selectedBookId ? response.data : user)));
        ...
        <Container>
            ...
            <Row style={styles.userBody}>
                    <Row style={styles.link}>
                        <a href='#000' style={styles.createLink} onClick={handleShow}>Add Streams</a>
                        ...
                        {show && (<div style={adding}>
                            <div style={addOuter}>
                                ...
                                <div style={modalBorder}>
                                    ...
                                    <div style={modalBorder}>
                                        <Form.Control type="text" placeholder="Stream" style={inputField} value={bookName} onChange={(e)=>setBookName(e.target.value)} />
                                    </div>
                                    <div style={modalBorder}>
                                    <Button variant="primary" onClick={handleCreateBook}   style={submitBtn}>
                                            Create
                                         </Button>
                                    </div>
                                </div>
                        ...
                        {showUpdateBook && (<div style={adding}>
                            <div style={addOuter}>
                                ...
                                <div style={modalBorder}>
                                    ...
                                    <div style={modalBorder}>
                                        <Form.Control type="text" placeholder="Book" style={inputField} value={updateName} onChange={(e)=>setUpdateName(e.target.value)} />
                                    </div>
                                    <div style={modalBorder}>
                        ...
                        {books&& books.length>0 && (
                            ...
                            <div style={styles.table}>
                                {books.map((book,index) => (
                                    <div className='rows' style={{ ...styles.rows, marginRight: '50px' }} key={index}>
                                    ...
                                    <div className='col' style={styles.col2}>
                                        <h6 style={{fontSize:'15px',marginBottom:'-2px',marginTop:'6px',fontStyle:'bold'}}  className='admin'>{book.name}</h6>
                                         <p style={{fontSize:'10px',textAlign:'left'}}>Book Stream</p>
                                    </div>
                                    <div className='col3' style={styles.col3}>
                                        <a  style={{ ...styles.funcLink ,color: book.blocked ? 'red' : 'grey' }}  onClick={() => handleRestrict(book._id)} >
                                            <BiBlock />&nbsp;&nbsp;&nbsp;
                                         </a>
                                        <a style={styles.funcLink} onClick={() => handleDeleteBook(book._id)}>
                                            <AiFillDelete />
                                        </a>
                                        <a style={{ ...styles.funcLink, marginLeft: '15px' }}  onClick={()=>fetchUserForUpdate(book._id,book.blocked)}>
                                            <RxUpdate />
                                 </a>
                                    </div>
