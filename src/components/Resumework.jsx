...
import {
  ...
  getUserID,
  assigned,
  getResumeLessonByUserId,
  getIdBybookName,
  getLessonByUserIdAndBookID,
  getBookByName,
  getLessonByUserIdAndBookIDResume,
  getBookName,
} from "../api/auth";
...
const Resumework = () => {
  ...
  const handleResume = async (name) => {
    ...
    const lesson = await getLessonByUserIdAndBookIDResume(
      Cookies.get("id"),
      res[0]._id
    );
...
&releText=${encodeURIComponent(lesson.data.relevance_to_subject)}
 ...
 &id=${encodeURIComponent(lesson.data._id)}&name=${encodeURIComponent(
      name
    )}&operation=resume&bk=${encodeURIComponent(
      lesson.data.book_id
    )}&days=${encodeURIComponent(lesson.data.days)}`);
  };
