...
import {createLesson,updateLesson,getClassData,getBook,getPublicationData,getBoardData,getBookName,getSKills,getCareer,getSKillContent,getCareerContent,getConceptualQuestion} from '../api/auth';
...
const Extra= () => {
  ...
  const [serviceList, setServiceList] = useState([{ service: '' },
    ...
    const [currentlyEditingIndexRele, setCurrentlyEditingIndexRele] = useState(-1);
    ...
    const [currentlyEditingIndexEvent, setCurrentlyEditingIndexEvent] = useState(-1);
  ...
  console.log("dayssss",lessonData.days)
    ...
    useEffect(()=>{
      ...
      else if(activeButton==='Informative'&&activeQButton>=serviceQList.length){
          setServiceQList([...serviceQList,{'qservice':''}]);
    }
    else if(activeButton==='Colearning'&&activeQButton>=serviceQList.length){
      setServiceQList([...serviceQList,{'qservice':''}]);
}
...
if((skillGain&& lid) || callSkillApi ){
  ...
  useEffect(()=>{
    ...
    useEffect(()=>{
      ...
      },[activeButton,informativeQuestionList,colearningQuestionList,conceptualQuestionList,activeButtonIndex])
...
const handleApplication = async (e) => {
  ...
  try {
    const response = await fetch("http://127.0.0.1:5000/upload_application", {
      method: "POST",
      body: formData,
    });
...
const handleProblem= async (e)=>{
  ...
  if (selectedProblem) {
    ...
    const updated = problem.map((item) =>
      item === selectedProblem ? { ...item, about: problemAbout, content: problemContent,description:problemDescription, ...(problemImage ? { image: problemImage } : {}), } : item
    );
...
const handleSkills=(e)=>{
  ...
  if (selectedSkill) {
    ...
    const updated = skillGain.map((item) =>
      item === selectedSkill ? { ...item,point: skillTitle, content: skillContent,description:skillDescription } : item
    );
    // Update the application list with the modified item
...
const handleUpdate=async(manual = true,event)=>{
  ...
  setIsManualUpdate(manual); console.log(manual)
...
const checkIfAllFieldsAreCompleted = () => {
  console.log(title,objective,content,application,releText,careerPath,events,skillGain,problem)
  if (title === '' || objective === '' || content === ''|| application=== ''|| releText==='' || careerPath==='' ||events=== ''|| skillGain==='' || problem===''  ) {
...
const handleAddLesson = async (e) => {
  ...
  try{
      ...
      if (!isCompleted) {
          ...
          if(searchParams.get('userName') && searchParams.get('operation')==="admin" ){
              const newD={
                  title:title?title:'',
                  ...
                  events_problem:'',
                  career_path:'',
                  last_interaction_with:Cookies.get('umail')?Cookies.get('umail'):'',
          ...
          else  if(searchParams.get('userName') && searchParams.get('operation')==="complete" ){
              newLessonData = {
                  title:title,
                  objective:objective,
                  content:content,
        ...
        await createLesson(newLessonData);
      alert("lesson added")
      if(searchParams.get('operation')&& searchParams.get('operation')==="admin"){
...
const handleNextQuestion = useCallback((e) => {
  ...
  else if(selectedQuestionType==='mutlipleChoice' || selectedQuestionType==='arrangeOrdering') {
    const q = txtQuestion;
  ...
  <Form >
                ...
                <div className='lessonRow1'>
                    ...
                    <div className='r3c'>
                        ...
                        <textarea rows={1} style={{ resize: 'none' }} placeholder="Add title"className="txtDetail" value={title} onChange={(event) => setTitle(event.target.value)} />
                            </InputGroup>
                        </div>
                    </div>
    ...
    />
                   ...
                   <div className='showApp' >
                       <h5>Application</h5>
                       {showApp&&(<button className='showicon' onClick={()=>setShowApp(false)}>-</button>)}
                       {!showApp&&(<button className='showicon' onClick={()=>setShowApp(true)}>+</button>)}
                   </div>
                   {
  showApp && (
    <div className="lessonRow5">
      ...
      <div className="row2">
        ...
        {imageUploadedApp ? (
          <img
      src={selectedApp?applicationImage:URL.createObjectURL(applicationImage)}  // Use createObjectURL to generate a URL for the image
      alt="Uploaded"
...
<div className="showApp">
  ...
  {showRele && (
    <button className="showiconRele" onClick={() => setShowRele(false)}>
      -
    </button>
  )}
  {!showRele && (
    <button className="showiconRele" onClick={() => setShowRele(true)}>
      +
    </button>
  )}
</div>
      ...
      <div className="row2">
        <div className="r6c2">
          <input
            ...
            placeholder="Point"
            value={releTitle}
            onChange={(e) => handleTextAreaChange(e, setReleTitle)}
          />
          <textarea
            placeholder="Content"
            rows={3}
            ...
            {releText &&
              releText.map((ele, index) => (
                ...
                >
                  <div className="description">
                    <h5>{ele.title}</h5>
                    <p style={{ marginLeft: "0px" }}>{ele.content}</p>
                  </div>
                </div>
              ))}
          ...
          <div className='eachCol'>
            <div className='description'>
              ...
              <p style={{marginLeft:"0px"}}>dbhbvhdbhvbhvghvg fff vgh hghf</p>
                    ...
                    <div className='showApp' >
                       <h5>Skill</h5>
                       {showSkills&&(<button className='showiconSkills' onClick={()=>setShowSkills(false)}>-</button>)}
                       {!showSkills&&(<button className='showiconSkills' onClick={()=>setShowSkills(true)}>+</button>)}
                   </div>
                    
                  {showSkills &&  <div className='lessonRow7'>
                    <div className='row1'>
                      <h4>Skill</h4>
                    </div>
                    <div className='row2'>
...
<input
      ...
      <div className="suggestions">
        {suggestedSkills.map((suggestedSkill, index) => (
          <p
            key={index}
            onClick={() => handleSuggestedSkillSelect(suggestedSkill)}
          ...
          >
            {suggestedSkill}
                            ...
                            <input type='text' placeholder='Description'   value={skillDescription} onChange={(e)=>{setSkillDescription(e.target.value)}}/>
                          <textarea placeholder='Content' rows={3} value={skillContent} onChange={(e)=>handleTextAreaChange(e,setSkillContent)}></textarea>
                          <button className='applicationAdd' onClick={(e)=>handleSkills(e)}>{selectedSkill ? 'Update' : 'Add'}</button>
                       </div>
                       <div className="r9c3">
                      <h5>Added Skills</h5>
                      <div className="outer">
                        <div className="skillRow">
                          ...
                          {skillGain &&
                              skillGain.map((idx, index) => (
                                <div className="eachCol"  key={index} onClick={() => handleItemClickSkill(index)}>
                                  <div className="description">
                                    <h5>{idx.point}</h5>
                                    <p>{idx.description}</p>
                    ...
                    <div className='showApp' >
                       <h5>Events</h5>
                       {showEvents&&(<button className='showiconEvents' onClick={()=>setShowEvents(false)}>-</button>)}
                       {!showEvents&&(<button className='showiconEvents' onClick={()=>setShowEvents(true)}>+</button>)}
                   </div>
  ...
  showEvents && (
    <div className="lessonRow5">
      ...
      <div className="row2">
        <div className="r5c1">
        {imageUploadedEvent ? (
    <img src={selectedEvent?eventImage:URL.createObjectURL(eventImage)} alt="Uploaded" className="uploaded-image" style={{
                        width: "17vh",
                        height: "20vh",
                        marginLeft:"-16px",
                        marginTop:"-15px",
              ...
              {events &&
                events.map((i, index) => (
                  ...
                  >
                    ...
                    <div className='showApp' >
                       <h5>Problem</h5>
                       {showProblem&&(<button className='showiconProb' onClick={()=>setShowProblem(false)}>-</button>)}
                       {!showProblem&&(<button className='showiconProb' onClick={()=>setShowProblem(true)}>+</button>)}
                   </div>

                   {showProblem && <div className='lessonRow7'>
                    <div className='row1'>
                      <h4>Problem</h4>
                      <label htmlFor="fileInput">
                        <FontAwesomeIcon
                    ...
                    <div className='row2'>
                       ...
                       <input type='text' placeholder='About'   value={problemAbout} onChange={(e)=>{setProblemAbout(e.target.value)}}/>
                         <input type='text' placeholder='Description'   value={problemDescription} onChange={(e)=>{setProblemDescription(e.target.value)}} />
                          <textarea placeholder='Content' rows={3} value={problemContent} onChange={(e)=>handleTextAreaChange(e,setProblemContent)}></textarea>
                          <button className='applicationAdd' onClick={(e)=>handleProblem(e)}>{selectedProblem ? 'Update' : 'Add'}</button>
                       </div>
                       <div className='r7c3'>
                          <h5>Added Item</h5>
                          ...
                          <div className='outer'> 
                          <div className="skillRow">
    {problem && problem.map((i, index) => (
     
        <div className='eachCol' key={index} onClick={() => handleItemClickProblem(index)}>
        {i.image && (<img src={`http://127.0.0.1:5000/static/${i.image}`} style={{
                        width: "70px",
                        height: "70px",
                        borderRadius: "20px",
                      }}/>)}
        ...
        <div className='description'>
          ...
          <div className="problem-content">
                    ...
                    <div className='showApp' >
                       <h5>Career</h5>
                       {showCareer&&(<button className='showiconEvents' onClick={()=>setShowCareer(false)}>-</button>)}
                       {!showCareer&&(<button className='showiconEvents' onClick={()=>setShowCareer(true)}>+</button>)}
                   </div>


                {showCareer &&    <div className="lessonRow10">
                  <div className="row1">
                    <h4>Career path </h4>
                    <label htmlFor="fileInput">
                        <FontAwesomeIcon
      ...
      <div className="suggestions">
        {suggestedCareer.map((suggested, index) => (
          <p
            key={index}
            onClick={() => handleSuggestedCareerSelect(suggested)}
            className='suggestionOption'
          >
            {suggestedCareer}
          </p>
                      ...
                      <div className="career-info">
                        <div className="career-grid">
                          ...
                          {careerSteps && careerSteps.map((item, index) => (
        <div key={index}>
        <input
                            type="text"
                            value={item.step}
  ...
  <div key={index}> 
    ...
    />
      ...
      <input
                        ...
                        <div className="image">
                          <button className="career-image-button fa-6x" onClick={(e)=>handleSteps(e)}>
                            +
                          </button>
                        </div>
                      </div>
                    ...
                    <div className="r8c3">
                      ...
                      <div className="outer">
                        <div className="skillRow" style={{marginTop:'4vh',marginLeft:'0.5vh'}}>
                        {careerPath.map((career, index) => (
    <div key={index} className='careerRight'> {/* Added key attribute */}
      ...
      {career.careerStep.map((e, stepIndex) => ( 
        ...
        <h6 style={{marginTop:'-0.3vh'}}>{e.step}</h6>
        
        <p style={{marginTop:'-0.5vh',paddingTop:'-1px',marginBottom:'0.5vh',marginLeft:'-1vh',marginRight:'-1vh'}}> _._ </p>
        <h6 style={{marginTop:'-0.5vh',paddingTop:'-1px',marginLeft:'-1vh'}}>{e.year}</h6>
        </div>
        <p>-</p>
        </div>

      ))}
      <img src={`http://127.0.0.1:5000/static/${career.image}`} style={{width:'40px',height:'40px',borderRadius:'50%'}}/>
    </div>
  ))}
                        </div>
                      </div>
                        ...
                        <div className="truefalse-ans-section">
                          ...
                          <div className="middle-section">
                            <input
                              ...
                              style={{borderBottom:'2px solid gray',color:'gray'}}
                            />
                  ...
                  <div className='b2row9'>
                                 ...
                                 <div className='row9c'>
                                 <button className='finishbutton' onClick={handleButtonActiveClick}>Finish</button>
                                 </div>
                            </div>
                  </div>
               ...
               <div className="question-view">
                  ...
                  {selectedQuestionList && selectedQuestionList.map((item, index) => (
                    <div>
                      <h5 className='questionViewQuestion'>   <FontAwesomeIcon icon={faCircle} />  {item.ques}</h5>
                      {(item.aType==='shortAnswer' || item.aType==='fillup' || item.aType==='text'|| item.aType==='trueNFalse') &&<div className="answerCss" dangerouslySetInnerHTML={{ __html: item.ans }} />}
                      {
                        (item.aType==='mutlipleChoice' || item.aType==='arrangeOrdering') && 
                       ( 
                        <div>
                        <h5>options</h5>
                        <li><div className="answerCss" dangerouslySetInnerHTML={{ __html: item.options.option1 }} /></li>
                        {item.options.option1Image && <img src={`http://127.0.0.1:5000/static/${item.options.option1Image}`} style={{height:'80px',width:'80px'}}/>}
                        <li><div className="answerCss"  dangerouslySetInnerHTML={{ __html: item.options.option2 }} /></li>
                        {item.options.option2Image && <img src={`http://127.0.0.1:5000/static/${item.options.option2Image}`} style={{height:'80px',width:'80px'}}/>}
                        {item.options.option3&& <div className="answerCss"  dangerouslySetInnerHTML={{ __html: item.options.option3 }} /> }
                        {item.options.option4&& <div className="answerCss"  dangerouslySetInnerHTML={{ __html: item.options.option4 }} /> }
                        
                        <div className="answerCss"  dangerouslySetInnerHTML={{ __html: item.selectedAns }} />
                        </div>
)                       
                      }
                    ...
                    <button className='completesave' onClick={(e) => {
      e.preventDefault(); // Prevent the default form submission
      handleUpdateLesson(false);
    }} style={{marginBottom:'-20px'}}>Save</button>
                    </div>}


                    { !data &&<div className='row10'> 
                    <button className='complete' onClick={handleAddLesson}>Complete</button>
                    </div>}
...
{data &&<div className='row10'> 
<button className='complete' onClick={(event)=>handleUpdate(true,event)} style={{backgroundColor:"darkred",border:'2px solid darkred'}}>Complete</button>
</div>}
</div>
