    ...
    const handleDeleteUser = async (userId) => {
      ...
      const handleUpdate=async()=>{
        try{
            ...
            console.log(response);
            setUsers((prevUsers) => prevUsers.map((user) => (user._id === selectedUserId ? response.data : user)));
            setShowUpdate(false);
        }
        catch(err){
            console.log(err)
                        ...
                        {show && (<div style={adding}>
                            <div style={addOuter}>
                                ...
                                <div style={modalBorder}>
                                    ...
                                    <div style={modalBorder}>
                                        <Form.Control type="text" placeholder="Name" style={inputField} value={name} onChange={(e) => setName(e.target.value)}/>
                                    </div>
                                   
                                    <div style={modalBorder}>
                                        <Form.Control type="email" placeholder="User Id" style={inputField} value={email} onChange={(e) => setEmail(e.target.value)} />
                                    </div>
                                    <div style={modalBorder}>
                                        <Form.Control type="Password" placeholder="Password" style={inputField} value={password} onChange={(e) => setPassword(e.target.value)} />
                                    </div>
                                    <div style={modalBorder}>
                                        <Button variant="primary"  onClick={handleCreateUser} style={submitBtn}>
                                             Create
                                         </Button>
                                    </div>
                                </div>
                   ...
                   {showUpdate && (<div style={adding}>
                            <div style={addOuter}>
                                ...
                                <div style={modalBorder}>
                                    ...
                                    <div style={modalBorder}>
                                        <Form.Control type="Password" placeholder="Password" style={inputField} value={updatePassword} onChange={(e) => setUpdatePassword(e.target.value)} />
                                    </div>

                                    <div style={modalBorder}>
                                        <Form.Control type="email" placeholder="Email" style={inputField} value={updateEmail} onChange={(e) => setUpdateEmail(e.target.value)} />
                                    </div>
                                    <div style={modalBorder}>
                   ...
                   <div className='userTable'>
                       ...
                       {users.length>0 &&
                             <div>
                                ...
                                <div className='userCol3' style={{width:'30vh',marginLeft:'-10vh'}}>
                                    <h6  className='admin'>{user.email}</h6>
                                    <p>User Id</p>
                                </div>
                                <div className='userCol4'>
                             <a  style={{ ...styles.funcLink, marginLeft: '15px' ,color: user.blocked ? 'red' : 'grey', }} onClick={()=>handleRestrict(user._id)}>
                                    <BiBlock />&nbsp;&nbsp;&nbsp;
                                </a>
                                <a  style={styles.funcLink} onClick={() => handleDeleteUser(user._id)}>
                                    <AiFillDelete />
                                </a>
                                <a style={{ ...styles.funcLink, marginLeft: '15px' }}  onClick={()=>fetchUserForUpdate(user._id,user.blocked)}>
                                            <RxUpdate />
                                 </a>
                          </div>
                                </div>
