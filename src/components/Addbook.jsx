...
const Addbook = () => {
  ...
  const handleUpdate = async () => {
    ...
    if (res) {
      setBooks((prevUsers) =>
        prevUsers.map((user) => (user._id === updateId ? res.data : user))
      );
    }
    setUpdateId("");
    setUpdateName("");
  ...
  useEffect(() => {
    const fetchClassNames = async () => {
      const classNames = await Promise.all(
        books.map((book) => getClassName(book.class_id))
      );
    ...
    const fetchPublisherNames = async () => {
      const publisherNames = await Promise.all(
        books.map((book) => getPublicationName(book.publication_id))
      );
    ...
    const fetchStreamNames = async () => {
      const streamNames = await Promise.all(
        books.map((book) => getStreamNames(book.stream_id))
      );
                ...
                <div className="adding">
                  <div className="addOuter">
                    ...
                    <div>
                      ...
                      <select
                        className="dropDownOptions"
                        value={updateStreamId}
                        onChange={(e) => setUpdateStreamId(e.target.value)}
                      >
                      ...
                      <select
                        className="dropDownOptions"
                        value={updatePublisherId}
                        onChange={(e) => setUpdatePublisherId(e.target.value)}
                      >
              ...
              <div className="userTable">
                {books.map((b, index) => (
                  <div className="userRow" key={index}>
                    <div className="userCol1" style={{ marginLeft: "20px" }}>
