...
import { assigned ,getUser,getBookByUserId,getAssignLessonByUserId,getBookByName,getLessonByUserIdAndBookIDAssign,getBookName} from '../api/auth';
...
const Assignedplan = () => {
  ...
  const handleAdd = async(v) => {
    ...
    const lesson=await getLessonByUserIdAndBookIDAssign(Cookies.get('id'),findBookId[0]._id);
   ...
   &id=${encodeURIComponent(lesson.data._id)}&name=${encodeURIComponent(v)}&operation=assign&informativeQues=${encodeURIComponent(lesson.data.informativeQues)}&bk=${encodeURIComponent(lesson.data.book_id)}`);
                          ...
                          <div className='col' style={styles.column}>
                            ...
                            /></div>
                             <div className='col' style={styles.col2}>
                                ...
                                <h6 className="admin"style={{fontSize:'15px',marginBottom:'-2px',marginTop:'6px',fontStyle:'bold',textAlign:'center'}}>{l}</h6>
                                <p style={{fontSize:'10px'}}>Book name</p>
{console.log("shhs")}
                                </div>
                                <div className='col3' style={styles.col3}>
                                <a  style={{textDecoration:'none',color:'blue'}}  onClick={()=>handleAdd(l)}>Start</a>
                            </div>
