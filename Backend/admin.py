...
def upload_application():
    ...
    if image and allowed_file(image.filename):
        ...
        return jsonify({"message": "Application uploaded successfully."}), 200
    else:
...
def upload_problem():
    ...
    if image and allowed_file(image.filename):
        ...
        filename=f"{ObjectId()}{filename}"
        image.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
        return jsonify({"message": "Problem uploaded successfully."}), 200
    else:
        return jsonify({"message": "Invalid image or file format."}), 400
...
def upload_career():
    ...
    if image and allowed_file(image.filename):
        ...
        filename=f"{ObjectId()}{filename}"
        image.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
        return jsonify({"message": "Career uploaded successfully."}), 200
    else:
        return jsonify({"message": "Invalid image or file format."}), 400
